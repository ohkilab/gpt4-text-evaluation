"""
    Sample Usage:
        python gpt4_group_criteria_batch.py --text_dir "./input" --result_base_dir "./output_json_files"
"""
...
def write_summary_to_csv(summary_data, output_path):
    df = pd.DataFrame(summary_data)
    df.to_csv(output_path, index=False, encoding='utf-8')

#load the arguments
parser=argparse.ArgumentParser()
parser.add_argument('--text_dir', type=str, required=True, help='評価対象のテキストファイルが格納されているディレクトリ')
parser.add_argument('--result_base_dir', type=str, default="./output_json_files", help="出力ファイルの保存ディレクトリ")
args=parser.parse_args()

# 入力ディレクトリと出力ディレクトリのパスを解決
text_dir = os.path.join(SCRIPT_DIR, args.text_dir.lstrip('./'))
...
criteria_path = os.path.join(SCRIPT_DIR, 'criteria.json')
...
for file in os.listdir(text_dir):
    if file.endswith('.txt'):
        ...
        cleaned_filename = file.replace('.txt', '')
        ...
        total_scores=[]
        criteria_scores = {cr: [] for cr in criteria}
        parsed_jsons = []
        ...
        for i in range(3):
            ...
            for cr in criteria:
                ...
                total_score += score
                criteria_scores[cr].append(score)
                print(f"Criteria: {cr},\n Score: {score},\n Explanation: {explanation}")

                time.sleep(0.1)
                
                write_to_json(parsed_json,f"{result_base_dir}/{cleaned_filename}_{'_'.join(cr.split())}_score_with_explanation_run{i+1}.json")

            print(f"Total Score: {total_score} of {5*len(criteria)}")
            total_scores.append(total_score)

        # 各criteriaの平均スコアを計算
        criteria_averages = {cr: sum(scores)/len(scores) for cr, scores in criteria_scores.items()}
        average_score = sum(total_scores) / len(total_scores)
        
        # 結果を辞書に格納
        result_dict = {
            'filename': cleaned_filename,
            **{f'{cr}_average': avg for cr, avg in criteria_averages.items()},
            'total_average': average_score
        }
        summary_data.append(result_dict)
        
        print(f"AVERAGE SCORE for {cleaned_filename}: {average_score} of {5*len(criteria)}")

# 結果をCSVファイルに出力
write_summary_to_csv(summary_data, f"{result_base_dir}/evaluation_summary.csv") 